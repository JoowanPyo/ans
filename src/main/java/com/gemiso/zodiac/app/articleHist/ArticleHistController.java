...
public class ArticleHistController {
    ...
    @GetMapping(path = "")
    public ApiResponse<List<ArticleHistDTO>> findAll(@Parameter(name = "artclId", description = "기사 아이디")
        ...
        return new ApiResponse<>(articleHistDTOList);
    }

    @Operation(summary = "기사이력 상세조회", description = "기사이력 상세조회")
    @GetMapping("/{artclHistId}")
    public ApiResponse<ArticleHistDTO> find(@Parameter(name = "artclHistId", required = true, description = "기사이력 아이디")
                                            @PathVariable("artclHistId") long artclHistId) {

        ArticleHistDTO articleHistDTO = articleHistService.find(artclHistId);

        return new ApiResponse<>(articleHistDTO);
    }
