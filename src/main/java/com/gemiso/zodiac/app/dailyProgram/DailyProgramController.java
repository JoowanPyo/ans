...
public class DailyProgramController {
    ...
    @GetMapping(path = "/{id}")
    public ApiResponse<DailyProgramDTO> find(@Parameter(name = "id", required = true, description = "일일편성 아이디")
                                                 @PathVariable("id") long id){

        DailyProgramDTO dailyProgramDTO = dailyProgramService.find(id);

        return new ApiResponse<>(dailyProgramDTO);
    }
    
    @Operation(summary = "일일편성 등록", description = "일일편성 등록")
    @PostMapping(path = "")
    public ApiResponse<DailyProgramDTO> create(@Parameter(description = "필수값<br> ", required = true)
